...
export const DebugPanel: React.FC<DebugPanelProps> = ({ messages, isOpen, onClose }) => {
  ...
  const stats = useMemo(() => {
    ...
    return { total, succeeded, failed, avgExecutionTime, avgMcpLatency };
  }, [filteredHistory]);
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
      <div className="bg-zinc-900 border border-zinc-700 rounded-lg w-full max-w-6xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-zinc-700">
          <h2 className="text-lg font-bold text-white">Debug Panel - Tool Execution History</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* Stats */}
            <div className="p-4 border-b border-zinc-700 grid grid-cols-5 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-400">
                  {stats.avgExecutionTime > 0 ? `${stats.avgExecutionTime.toFixed(0)}ms` : 'N/A'}
                </div>
                <div className="text-xs text-zinc-400">Avg Exec Time</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-400">
                  {stats.avgMcpLatency > 0 ? `${stats.avgMcpLatency.toFixed(0)}ms` : 'N/A'}
                </div>
                <div className="text-xs text-zinc-400">Avg MCP Latency</div>
              </div>
            </div>

            {/* History List */}
            <div className="flex-1 overflow-y-auto p-4">
              <h3 className="text-xs font-bold text-zinc-400 mb-2">TOOL CALL HISTORY</h3>
              {filteredHistory.length === 0 ? (
                <div className="text-center text-zinc-500 text-sm py-8">No tool calls recorded</div>
              ) : (
