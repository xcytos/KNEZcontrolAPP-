...
test.describe('Tauri Native Chat Suite - 4 Tests', () => {
  ...
  test('Test 1: Basic chat with assistant response', async ({ tauriPage }) => {
    ...
    // Send first message using data-testid selectors from documentation
    await tauriPage.locator('[data-testid="chat-input"]').fill('Hello, can you introduce yourself?');
    await tauriPage.waitForTimeout(500);
    await tauriPage.locator('[data-testid="send-button"]').click();
  ...
  test('Test 2: Chat with memory context - follow-up question', async ({ tauriPage }) => {
    ...
    await tauriPage.locator('[data-testid="chat-input"]').fill('What was my previous question?');
    await tauriPage.waitForTimeout(500);
    await tauriPage.locator('[data-testid="send-button"]').click();
  ...
  test('Test 3: Chat with specific topic and verification', async ({ tauriPage }) => {
    ...
    await tauriPage.locator('[data-testid="chat-input"]').fill('Explain what Tauri is in one sentence');
    await tauriPage.waitForTimeout(500);
    await tauriPage.locator('[data-testid="send-button"]').click();
  ...
  test('Test 4: Chat with session memory verification', async ({ tauriPage }) => {
    ...
    await tauriPage.locator('[data-testid="chat-input"]').fill('How many messages have we exchanged in this session?');
    await tauriPage.waitForTimeout(500);
    await tauriPage.locator('[data-testid="send-button"]').click();
